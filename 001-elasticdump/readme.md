...
-----
```bash
$ docker build -t elasticdump:bw2017 .
```
...
```bash
$ docker run -t elasticdump:bw2017 --input=http://production.es.com:9200/my_index --output=$ | gzip > /data/my_index.json.gz
```

Backup the results of a query to a file:

```bash
$ docker run -t elasticdump:bw2017 --input=http://production.es.com:9200/my_index --output=query.json --searchBody '{"query":{"term":{"username": "admin"}}}'
```
